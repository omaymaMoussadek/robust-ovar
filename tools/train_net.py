...
def _resolve_checkpoint_state(checkpoint, cfg, scope):
    ...
    assert pathmgr.exists(base_file), "Base checkpoint '{}' not found".format(base_file)
...
def train_epoch(
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(
        ...
        with torch.cuda.amp.autocast(enabled=cfg.TRAIN.MIXED_PRECISION):
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel":
                ...
                elif cfg.MODEL.KEEP_RAW_MODEL and (
                    cfg.MODEL.RAW_MODEL_DISTILLATION or cfg.MODEL.MSE_MODEL_DISTILLATION
                ):
                    if cfg.MODEL.MSE_MODEL_DISTILLATION:
                        preds, raw_preds, emb_preds, emb_raw = model(inputs)
                    else:
            ...
            if cfg.MODEL.MODEL_NAME == "ContrastiveModel" and partial_loss:
                ...
                if cfg.MODEL.KEEP_RAW_MODEL and cfg.MODEL.MSE_MODEL_DISTILLATION:
                    emb_mse_loss = F.mse_loss(emb_preds, emb_raw)
                    if cur_iter % cfg.LOG_PERIOD == 0:
                        logger.info('Embedding MSE Loss: %.8f'%emb_mse_loss.item())
                        logger.info('Embedding MSE Ratio: %f'%cfg.MODEL.EMB_MSE_RATIO)
                    loss += cfg.MODEL.EMB_MSE_RATIO * emb_mse_loss
        ...
        if cfg.DETECTION.ENABLE:
            ...
            #         {
            #             "train/loss": loss,
            #             "train/top1": 100.0 - top1_err,
            #             "train/top5": 100.0 - top5_err,
            #             "train/top1_err": top1_err,
            #             "train/top5_err": top5_err,
            #             "train/lr": lr,
    ...
    train_meter.log_epoch_stats(cur_epoch)
    # W&B preview:
    # epoch_stats = train_meter.log_epoch_stats(cur_epoch)
    # if wandb_run is not None and epoch_stats is not None:
    #     wandb_run.log(
    #         {
    #             "train_epoch/loss": epoch_stats["loss"],
    #             "train_epoch/top1": 100.0 - epoch_stats["top1_err"],
    #             "train_epoch/top5": 100.0 - epoch_stats["top5_err"],
    #             "train_epoch/top1_err": epoch_stats["top1_err"],
    #             "train_epoch/top5_err": epoch_stats["top5_err"],
    #             "epoch": cur_epoch + 1,
    #         },
    #         step=(cur_epoch + 1) * data_size,
    #     )
    train_meter.reset()
...
def eval_epoch(
    ...
    for cur_iter, (inputs, labels, index, time, meta) in enumerate(val_loader):
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.DATA.MULTI_LABEL:
                ...
                #     wandb_run.log(
                #         {
                #             "val/top1": 100.0 - top1_err,
                #             "val/top5": 100.0 - top5_err,
    ...
    # if wandb_run is not None and epoch_stats is not None and "top1_err" in epoch_stats:
    #     wandb_run.log(
    #         {
    #             "val_epoch/top1": 100.0 - epoch_stats["top1_err"],
    #             "val_epoch/top5": 100.0 - epoch_stats["top5_err"],
    #             "val_epoch/top1_err": epoch_stats["top1_err"],
    #             "val_epoch/top5_err": epoch_stats["top5_err"],
    #             "epoch": cur_epoch + 1,
    #         },
    #         step=(cur_epoch + 1) * len(val_loader),
    #     )
    # write to tensorboard format if available.
    if writer is not None:
        if cfg.DETECTION.ENABLE:
...
def train(cfg):
    ...
    if cfg.TRAIN.CUSTOM_LOAD: 
        ...
        checkpoint_model = _resolve_checkpoint_state(checkpoint, cfg, cfg.TRAIN)
    ...
    #         dir=cfg.OUTPUT_DIR,
    #         config={"output_dir": cfg.OUTPUT_DIR, "rng_seed": cfg.RNG_SEED},
    #     )
    # else:
    #     wandb_run = None
