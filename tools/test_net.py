...
def perform_test(
    ...
    for cur_iter, (inputs, labels, video_idx, time, meta) in enumerate(
        ...
        if cfg.DETECTION.ENABLE:
            ...
            if cfg.MODEL.RECORD_ROUTING:
                ...
                preds = model(inputs)
                # Some models (e.g., temporal CLIP with distillation) return
                # (pred, raw_pred, ...) during eval. For testing we only need
    ...
    test_meter.finalize_metrics()
    # W&B preview:
    # if wandb_run is not None and not cfg.DETECTION.ENABLE:
    #     test_top1 = float(test_meter.stats["top1_acc"])
    #     test_top5 = float(test_meter.stats["top5_acc"])
    #     wandb_run.log(
    #         {
    #             "test/top1": test_top1,
    #             "test/top5": test_top5,
    #             "test/top1_err": 100.0 - test_top1,
    #             "test/top5_err": 100.0 - test_top5,
    #         }
    #     )
    return test_meter
...
def test(cfg):
    ...
    for num_view in cfg.TEST.NUM_TEMPORAL_CLIPS:
        ...
        if cfg.TEST.CUSTOM_LOAD:
            ...
            checkpoint_model = _resolve_checkpoint_state(checkpoint, cfg, cfg.TEST)
