...
class TemporalClipVideo(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.mse_distillation = cfg.MODEL.MSE_MODEL_DISTILLATION

        if (self.distillation or self.mse_distillation) and (not self.keep_raw_model):
    ...
    def forward(self, x=None, update=False):
        ...
        if self.training:
            ...
            if self.keep_raw_model and (self.ensemble_pred or self.distillation or self.mse_distillation):
                with torch.no_grad():
                    ...
                    if self.mse_distillation:
                        img_embed = img_encode.reshape(bz, clip_len, -1).mean(1)
                        raw_img_embed = raw_img_encode.reshape(bz, clip_len, -1).mean(1)
                        return pred, raw_pred, img_embed, raw_img_embed
